...
public interface UserRepository extends JpaRepository<User, Long> {
    @Query(value = "SELECT c.*, ea.first_name, ea.last_name, ea.target, ea.username, ea.country_country_id\n" +
            "FROM users_confidential c, users_eaeu ea\n" +
            "WHERE c.user_id = ea.user_id\n" +
            "UNION ALL \n" +
            "SELECT c.*, we.first_name, we.last_name, we.target, we.username, we.country_country_id\n" +
    ...
    @Query(value = "SELECT c.*, we.first_name, we.last_name, we.target, we.username, we.country_country_id\n" +
            "FROM users_confidential c, users_weeu we\n" +
            "WHERE c.user_id = we.user_id", nativeQuery = true)
